...
int main ()
{
	uint8_t user_data = 0xAA;
	//char user_data[] = "Hello World";
	//uint8_t dummy_bite = 0xFF;
	//uint8_t ackbyte;
	GPIO_Button_Init();
	...
	while(1)
	{
		if(GPIO_ReadFromInputPin(GPIOD, GPIO_PIN_NO_0) == 0)
		{
			...
			//SPI_SendData(SPI1,(uint8_t*)strlen(user_data),1);
			// send this before data to see if the slave support it
			//SPI_SendData(SPI1,&dummy_bite,1);


			//SPI_ReceiveData(SPI1, &ackbyte, 1);
			//if(SPI_VerifyResponse(ackbyte))
			//{
				//SPI_SendData(SPI1,(uint8_t*)user_data,(uint32_t)strlen(user_data));
